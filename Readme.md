...
- a **rebuilt Power BI report** running on that synthetic data, reproducing the original end to end

Clone it, run the generator, load the schema, and the whole thing runs on your machine. Working inside a data-governance constraint was part of the job, and this repo is built to be honest about it rather than around it.
...
*Reproduction of the DMRC report, rebuilt on synthetic data with matched distributions. The original was built at DMRC against live ATS data and cannot be shared. Build steps: [`docs/POWERBI_BUILD_GUIDE.md`](docs/POWERBI_BUILD_GUIDE.md)*
...
**Power BI only** — no database required. The three CSVs in `data/` are already
generated. Open Power BI Desktop, `Get Data → Text/CSV`, and follow
[`docs/POWERBI_BUILD_GUIDE.md`](docs/POWERBI_BUILD_GUIDE.md).

**Full pipeline**, if you want the PostgreSQL layer too:
...
The generator checks its own output against the real distributions:
...
No DMRC operational data — raw, sampled, aggregated or anonymised — is contained in this repository. Every row under `data/` is synthetically generated by `generate_sample.py`. Station and line names are public information printed on the network map, used here purely as readable labels.

The aggregate figures in *Findings* are published outcomes from the live build, and contain no passenger, transaction or station-level detail.
